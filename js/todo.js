...
let ToDo = function (rootElementAll, rootElementActive, rootElementCompleted) {
    ...
    function generateToDoItemView(toDoItem) {
        ...
        let dataElem = toDoItemRoot.getElementsByClassName('data')[0].innerHTML = toDoItem.content;
    ...
    function registerClickHandlers(toDoItemViewModel) {
        for(let i = 0; i < toDoItemViewModel.views.length; i++) {
            toDoItemViewModel.views[i].onclick = function(e) {
                ...
                let index = toDoItems.findIndex(item => item.data.id === id);
    ...
    function registerDeleteHandlers(toDoItemViewModel) {
        for(let i = 0; i < toDoItemViewModel.views.length; i++) {
            toDoItemViewModel.views[i].getElementsByClassName('delete-btn')[0].onclick = function (e) {
                e.stopPropagation();
                ...
                if(index > -1) {
                    ...
                    toDoItemViewModel.views[0].parentNode.removeChild(toDoItemViewModel.views[0]);
                    toDoItemViewModel.views[1].parentNode.removeChild(toDoItemViewModel.views[1]);
                }
    ...
    function loadFromLocalStorage() {
        let json = localStorage.getItem('todo-data');
